...
-> Albeo_Texture G buffer에 값이 제대로 담기고 있지 않고 있음, 정확히는, 텍스처가 제대로 안불러와지고 있는것으로 추정됨
// 같은 float4인 normal 버퍼로 Albeo_Texture를 전달하여, 렌더링 해봤지만, 제대로 안됨 + normal 은 제대로 됨

예상으로는 렌더 타겟에 덮어 씌워지고 있는 것 같은데..?

+

조명 설정에 따라 배경 화면의 색상도 달라지는 중 -> 왜?
