...
{
    matrix gmtxGameObject : packoffset(c0); // 16°³ (c0 ~ c3)
    Material_Info material_info : packoffset(c4); // 8°³ (c4 ~ c5)
    float3 gObjectVelocity : packoffset(c6); // 3°³ (c6.xyz)
    uint gnTexturesMask : packoffset(c6.w); // 1°³ (c6.w)
};
