...
VS_TRAIL_OUTPUT VSTrail(VS_TRAIL_INPUT input)
{
...
PS_MULTIPLE_RENDER_TARGETS_OUTPUT PSTrail(VS_TRAIL_OUTPUT input)
{
    PS_MULTIPLE_RENDER_TARGETS_OUTPUT output;

    // 간단한 알파 페이드 (시간 기반)
    float trailAge = input.time;
    float fade = saturate(1.0f - trailAge); // trailAge가 1에 가까우면 투명

    float4 trailColor = float4(1.0f, 0.2f, 0.1f, fade); // 예시 색상
    trailColor = float4(1.0f, 0.0f, 0.0f, 1.0f);
    output.Albedo_Color = trailColor;

    output.world_Position = float4(input.positionW, 1.0f);
    output.world_Normal_and_Camera_Distance = float4(0, 1, 0, 0); // 단순 노멀
    output.Material_Light_Info = float4(0, 0, 0, 0);
    output.Velocity_Mask_Obj_Id = float4(0, 0, 0, 0);
