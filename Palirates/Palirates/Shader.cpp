...
void CShader::CreateShader(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, shared_ptr<ID3D12RootSignature> pd3dGraphicsRootSignature)
{
}

void CShader::CreateShader(ID3D12Device* pd3dDevice, shared_ptr<ID3D12RootSignature> pd3dGraphicsRootSignature, UINT nRenderTargets, DXGI_FORMAT* pdxgiRtvFormats, DXGI_FORMAT dxgiDsvFormat)
{
}
...
void CStandardShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, shared_ptr<ID3D12RootSignature> pd3dGraphicsRootSignature)
{
	m_ngraphicsPipelineStates = 1;
	m_ppd3dgraphicsPipelineStates = new ID3D12PipelineState * [m_ngraphicsPipelineStates];

	CreateGraphicsPipelineState(pd3dDevice, pd3dGraphicsRootSignature.get(), 0); // 기본 그리기
}
...
void CStandard_Instance_Shader::CreateShader(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, shared_ptr<ID3D12RootSignature> pd3dGraphicsRootSignature)
{
	m_ngraphicsPipelineStates = 1;
	m_ppd3dgraphicsPipelineStates = new ID3D12PipelineState * [m_ngraphicsPipelineStates];
...
float Plane_Shader::Current_Time;
float Plane_Shader::Elapsed_Time;

Plane_Shader::Plane_Shader()
{
}
...
void Plane_Shader::CreateShader(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, shared_ptr<ID3D12RootSignature> pd3dGraphicsRootSignature)
{
	m_ngraphicsPipelineStates = 1;
	m_ppd3dgraphicsPipelineStates = new ID3D12PipelineState * [m_ngraphicsPipelineStates];
...
D3D12_INPUT_LAYOUT_DESC Plane_Shader::CreateInputLayout(int nPipelineState)
{
	UINT nInputElementDescs = 4;
	D3D12_INPUT_ELEMENT_DESC* pd3dInputElementDescs = new D3D12_INPUT_ELEMENT_DESC[nInputElementDescs];
...
D3D12_RASTERIZER_DESC Plane_Shader::CreateRasterizerState(int nPipelineState)
{
	D3D12_RASTERIZER_DESC d3dRasterizerDesc;
	::ZeroMemory(&d3dRasterizerDesc, sizeof(D3D12_RASTERIZER_DESC));
...
D3D12_BLEND_DESC Plane_Shader::CreateBlendState(int nPipelineState)
{
	D3D12_BLEND_DESC d3dBlendDesc = {};
	d3dBlendDesc.AlphaToCoverageEnable = FALSE;
	d3dBlendDesc.IndependentBlendEnable = FALSE;
...
D3D12_DEPTH_STENCIL_DESC Plane_Shader::CreateDepthStencilState(int nPipelineState)
{
	D3D12_DEPTH_STENCIL_DESC d3dDepthStencilDesc = {};
	d3dDepthStencilDesc.DepthEnable = TRUE;
	d3dDepthStencilDesc.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ZERO;
...
D3D12_SHADER_BYTECODE Plane_Shader::CreateVertexShader(ID3DBlob** VertexShaderBlob, int nPipelineState)
{
	if (nPipelineState == 0)
...
D3D12_SHADER_BYTECODE Plane_Shader::CreatePixelShader(ID3DBlob** PixelShaderBlob, int nPipelineState)
{
	if (nPipelineState == 0)
...
void Plane_Shader::CreateShaderVariables(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
...
void Plane_Shader::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
...
void Plane_Shader::ReleaseShaderVariables()
{
	if (Frame_Info)
	{
...
void Plane_Shader::Update(float ElapsedTime)
{
...
void Trail_Shader::CreateShader(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, shared_ptr<ID3D12RootSignature> pd3dGraphicsRootSignature)
{
	m_ngraphicsPipelineStates = 1;
	m_ppd3dgraphicsPipelineStates = new ID3D12PipelineState * [m_ngraphicsPipelineStates];
...
//===============================================================================
UINT PostProcessBaseShader::Frame_Buffer_Width = FRAME_BUFFER_WIDTH;
UINT PostProcessBaseShader::Frame_Buffer_Height = FRAME_BUFFER_HEIGHT;
...
ID3D12RootSignature* PostProcessBaseShader::CreateGraphicsRootSignature(ID3D12Device* pd3dDevice)
{
	D3D12_DESCRIPTOR_RANGE pd3dDescriptorRanges[5];
	{
		...
		pd3dDescriptorRanges[1].NumDescriptors = 1;  // Structured Buffer  == Light_Material_Info
	...
	D3D12_ROOT_PARAMETER pd3dRootParameters[9];
	{
		...
		pd3dRootParameters[ROOT_PARAMETER_FOG_INFO_INDEX].Descriptor.ShaderRegister = 0; // Fog_Info
		...
		pd3dRootParameters[ROOT_PARAMETER_POST_LIGHT_INFO_CBV_INDEX].Descriptor.ShaderRegister = 2; // Light_Info
		...
		pd3dRootParameters[ROOT_PARAMETER_POST_SHADOW_INFO_CBV_INDEX].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
		pd3dRootParameters[ROOT_PARAMETER_POST_SHADOW_INFO_CBV_INDEX].Descriptor.ShaderRegister = 3; // Shadow_Info
		pd3dRootParameters[ROOT_PARAMETER_POST_SHADOW_INFO_CBV_INDEX].Descriptor.RegisterSpace = 0;
		pd3dRootParameters[ROOT_PARAMETER_POST_SHADOW_INFO_CBV_INDEX].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
		...
		pd3dRootParameters[ROOT_PARAMETER_FOG_NOISE_TEXTURE_SRV_INDEX].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
		pd3dRootParameters[ROOT_PARAMETER_FOG_NOISE_TEXTURE_SRV_INDEX].DescriptorTable.NumDescriptorRanges = 1;
		pd3dRootParameters[ROOT_PARAMETER_FOG_NOISE_TEXTURE_SRV_INDEX].DescriptorTable.pDescriptorRanges = &pd3dDescriptorRanges[2]; // Fog Noise Texture
		pd3dRootParameters[ROOT_PARAMETER_FOG_NOISE_TEXTURE_SRV_INDEX].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;

		pd3dRootParameters[ROOT_PARAMETER_FIXED_SHADOWMAP_TEXTURE_SRV_INDEX].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	...
	pd3dSamplerDescs[1].MinLOD = 0.0f;
...
void PostProcessBaseShader::Set_ScreenSize(UINT new_width, UINT new_height)
{
...
D3D12_CLEAR_VALUE PostProcessBaseShader::Get_ClearValue_For_RTVFormat(DXGI_FORMAT format)
{
	...
	else if (format == DXGI_FORMAT_R8G8B8A8_UNORM) 
	{
		...
		clearValue.Color[2] = 1.0f;		clearValue.Color[3] = 0.0f;
	}
...
ScreenFade_Shader::ScreenFade_Shader()
{
}
...
void ScreenFade_Shader::CreateShader(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, shared_ptr<ID3D12RootSignature> pd3dGraphicsRootSignature)
{
	m_ngraphicsPipelineStates = 1;
	m_ppd3dgraphicsPipelineStates = new ID3D12PipelineState * [m_ngraphicsPipelineStates];
...
D3D12_INPUT_LAYOUT_DESC ScreenFade_Shader::CreateInputLayout(int nPipelineState)
{
	D3D12_INPUT_LAYOUT_DESC d3dInputLayoutDesc = {};
...
D3D12_SHADER_BYTECODE ScreenFade_Shader::CreateVertexShader(ID3DBlob** VertexShaderBlob, int nPipelineState)
{
	if (nPipelineState == 0)
		return CShader::CompileShaderFromFile(L"Deffered_Shaders.hlsl", "ScreenFade_VS", "vs_5_1", VertexShaderBlob);
	else
	{
		D3D12_SHADER_BYTECODE d3dShaderByteCode = { 0, NULL };
		return d3dShaderByteCode;
...
D3D12_SHADER_BYTECODE ScreenFade_Shader::CreatePixelShader(ID3DBlob** PixelShaderBlob, int nPipelineState)
{
	if (nPipelineState == 0)
		return CShader::CompileShaderFromFile(L"Deffered_Shaders.hlsl", "ScreenFade_PS", "ps_5_1", PixelShaderBlob);
	else
	{
		D3D12_SHADER_BYTECODE d3dShaderByteCode = { 0, NULL };
		return d3dShaderByteCode;
...
D3D12_RASTERIZER_DESC ScreenFade_Shader::CreateRasterizerState(int nPipelineState)
{
	D3D12_RASTERIZER_DESC d3dRasterizerDesc;
	::ZeroMemory(&d3dRasterizerDesc, sizeof(D3D12_RASTERIZER_DESC));
	d3dRasterizerDesc.FillMode = D3D12_FILL_MODE_SOLID;
...
D3D12_BLEND_DESC ScreenFade_Shader::CreateBlendState(int nPipelineState)
{
	D3D12_BLEND_DESC d3dBlendDesc;
	::ZeroMemory(&d3dBlendDesc, sizeof(D3D12_BLEND_DESC));
	d3dBlendDesc.AlphaToCoverageEnable = FALSE;
...
D3D12_DEPTH_STENCIL_DESC ScreenFade_Shader::CreateDepthStencilState(int nPipelineState)
{
	D3D12_DEPTH_STENCIL_DESC d3dDepthStencilDesc;
	::ZeroMemory(&d3dDepthStencilDesc, sizeof(D3D12_DEPTH_STENCIL_DESC));
	d3dDepthStencilDesc.DepthEnable = FALSE;
...
void CSkinnedAnimationStandardShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, shared_ptr<ID3D12RootSignature> pd3dGraphicsRootSignature)
{
	m_ngraphicsPipelineStates = 1;
	m_ppd3dgraphicsPipelineStates = new ID3D12PipelineState * [m_ngraphicsPipelineStates];
...
D3D12_DEPTH_STENCIL_DESC Deferred_CStandard_Shader::CreateDepthStencilState(int nPipelineState)
{
	D3D12_DEPTH_STENCIL_DESC d3dDepthStencilDesc;
	::ZeroMemory(&d3dDepthStencilDesc, sizeof(D3D12_DEPTH_STENCIL_DESC));
	if (nPipelineState == 0)
...
D3D12_RASTERIZER_DESC Deferred_CStandard_Shader::CreateRasterizerState(int nPipelineState)
{
	D3D12_RASTERIZER_DESC d3dRasterizerDesc;
	::ZeroMemory(&d3dRasterizerDesc, sizeof(D3D12_RASTERIZER_DESC));
...
void Deferred_CStandard_Shader::CreateGraphicsPipelineState(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dGraphicsRootSignature, UINT nRenderTargets, DXGI_FORMAT* pdxgiRtvFormats, DXGI_FORMAT dxgiDsvFormat, int nPipelineState)
{
	...
	else if (nPipelineState == 1) // shadow
	{
...
void Deferred_CStandard_Instance_Shader::CreateShader(ID3D12Device* pd3dDevice, shared_ptr<ID3D12RootSignature> pd3dGraphicsRootSignature, UINT nRenderTargets, DXGI_FORMAT* pdxgiRtvFormats, DXGI_FORMAT dxgiDsvFormat)
{
...
D3D12_SHADER_BYTECODE Deferred_CStandard_Instance_Shader::CreateVertexShader(ID3DBlob** VertexShaderBlob, int nPipelineState)
{
	if (nPipelineState == 0)
		...
		return CompileShaderFromFile(L"Shaders.hlsl", "VS_Shadow_Standard_INSTANCE", "vs_5_1", VertexShaderBlob);
	else
	{
		D3D12_SHADER_BYTECODE d3dShaderByteCode = { 0, NULL };
		return 		d3dShaderByteCode;
...
void Deferred_CTerrainShader::CreateShader(ID3D12Device* pd3dDevice, shared_ptr<ID3D12RootSignature> pd3dGraphicsRootSignature, UINT nRenderTargets, DXGI_FORMAT* pdxgiRtvFormats, DXGI_FORMAT dxgiDsvFormat)
{
	m_ngraphicsPipelineStates = 2;
	m_ppd3dgraphicsPipelineStates = new ID3D12PipelineState * [m_ngraphicsPipelineStates];

	CreateGraphicsPipelineState(pd3dDevice, pd3dGraphicsRootSignature.get(), nRenderTargets, pdxgiRtvFormats, dxgiDsvFormat, 0); // PSO - 0 solid
	CreateGraphicsPipelineState(pd3dDevice, pd3dGraphicsRootSignature.get(), nRenderTargets, pdxgiRtvFormats, DXGI_FORMAT_D32_FLOAT, 1); // PSO - 1 For ShadowMap
}
...
D3D12_SHADER_BYTECODE Deferred_CTerrainShader::CreateVertexShader(ID3DBlob** VertexShaderBlob, int nPipelineState)
{
	if (nPipelineState == 0)
		...
		return CompileShaderFromFile(L"Shaders.hlsl", "VS_Shadow_Terrain", "vs_5_1", VertexShaderBlob);
	else
	{
		D3D12_SHADER_BYTECODE d3dShaderByteCode = { 0, NULL };
		return 		d3dShaderByteCode;
...
{
	if (nPipelineState == 0)
		return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PSTerrain", "ps_5_1", PixelShaderBlob));
	else
	{
		D3D12_SHADER_BYTECODE d3dShaderByteCode = { 0, NULL };
		return 		d3dShaderByteCode;
...
void Deferred_Plane_Shader::CreateShader(ID3D12Device* pd3dDevice, shared_ptr<ID3D12RootSignature> pd3dGraphicsRootSignature, UINT nRenderTargets, DXGI_FORMAT* pdxgiRtvFormats, DXGI_FORMAT dxgiDsvFormat)
{
	m_ngraphicsPipelineStates = 2;
	m_ppd3dgraphicsPipelineStates = new ID3D12PipelineState * [m_ngraphicsPipelineStates];

	CreateGraphicsPipelineState(pd3dDevice, pd3dGraphicsRootSignature.get(), nRenderTargets, pdxgiRtvFormats, dxgiDsvFormat, 0);
	CreateGraphicsPipelineState(pd3dDevice, pd3dGraphicsRootSignature.get(), nRenderTargets, pdxgiRtvFormats, DXGI_FORMAT_D32_FLOAT, 1); // PSO - 1 For ShadowMap
}
...
{
	if (nPipelineState == 0)
		return(CShader::CompileShaderFromFile(L"Plane.hlsl", "PS_Deffered_Plane", "ps_5_1", PixelShaderBlob));
	else
	{
		D3D12_SHADER_BYTECODE d3dShaderByteCode = { 0, NULL };
		return 		d3dShaderByteCode;
...
void Deferred_CSkyBoxShader::CreateShader(ID3D12Device* pd3dDevice, shared_ptr<ID3D12RootSignature> pd3dGraphicsRootSignature, UINT nRenderTargets, DXGI_FORMAT* pdxgiRtvFormats, DXGI_FORMAT dxgiDsvFormat)
{
	m_ngraphicsPipelineStates = 1;
	m_ppd3dgraphicsPipelineStates = new ID3D12PipelineState * [m_ngraphicsPipelineStates];
