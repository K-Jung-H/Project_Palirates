...
void OBBCollision_Manager::Update_OBB_Data(const std::vector<shared_ptr<CGameObject>>& obj_list)
{
	...
	{
		if (!obj_ptr || !obj_ptr->m_pMesh || !obj_ptr->Get_Active()) continue;
...
std::vector<OBB_Info> OBBCollision_Manager::Check_OBB_Collisions_ByCell(const BoundingOrientedBox& obb) const
{
...
bool OBB_Manager::Check_OBB_Collision(const BoundingOrientedBox& obb) const
{
	vector<OBB_Info> collided_obb_list = collision_manager->Check_OBB_Collisions_ByCell(obb);
...
//
//		std::vector<OBB_Info> collisions = collision_manager->Check_OBB_Collisions_ByCell(testOBB);
...
//
//	std::vector<OBB_Info> stillColliding = collision_manager->Check_OBB_Collisions_ByCell(finalOBB);
...
{
	std::vector<OBB_Info> stillColliding = collision_manager->Check_OBB_Collisions_ByCell(playerOBB);
	if (stillColliding.empty())
		return XMVectorZero();
...
void Object_Manager::Check_Player_Collision(shared_ptr<CPlayer> player_ptr)
{
	...
	Check_Dynamic_OBB_Collision(worldOBB);
	Check_Fixed_OBB_Collision(player_ptr);
}

void Object_Manager::Check_Dynamic_OBB_Collision(const BoundingOrientedBox& obb)
{
	dynamic_obb_manager->Check_OBB_Collision(obb);
}
...
void Object_Manager::Check_Fixed_OBB_Collision(const shared_ptr<CGameObject> obj_ptr)
{
	...
	// Calculate player's OBB in world space
	BoundingOrientedBox worldOBB;
	localOBB->Transform(worldOBB, XMLoadFloat4x4(&player->m_xmf4x4World));
	...
	// Calculate the sliding direction for collision (updates moveDir)
	bool slid = fixed_obb_manager->Calc_Slide_Move(worldOBB, moveDir, 0.0001f, 4);

	// Store the sliding direction for EnableSliding
	XMFLOAT3 slideDir;
	XMStoreFloat3(&slideDir, moveDir);

	// Move the OBB by the sliding direction (before overlap resolution)
	XMVECTOR newCenter = XMLoadFloat3(&worldOBB.Center) + moveDir;
	XMStoreFloat3(&worldOBB.Center, newCenter);

	// Check and resolve any remaining overlap (pushBack correction)
	XMVECTOR pushBack = fixed_obb_manager->Resolve_Overlap(worldOBB);

	if (!XMVector3Equal(pushBack, XMVectorZero()))
	{
		// Apply pushBack correction if still overlapping after sliding
		newCenter += pushBack;
		XMStoreFloat3(&worldOBB.Center, newCenter);
	}

	// Set player's final position after overlap resolution
	XMFLOAT3 finalPos;
	XMStoreFloat3(&finalPos, newCenter);
	player->SetPosition(finalPos);

	// Pass the sliding direction (before overlap correction) to the player
