...
private:
    ...
    std::unordered_map<int, std::shared_ptr<CPlayer>> remote_players;
...
public:
    ...
    std::shared_ptr<CTerrainPlayer> GetRemotePlayer(int playerId);

    const std::unordered_map<int, std::shared_ptr<CPlayer>>& GetAllRemotePlayers() const {return remote_players;}
