...
//서버 추가
void Object_Manager::AddPlayer(Player* player)
{
    skinned_object_list.push_back(player);
}

void Object_Manager::UpdatePlayerPosition(int playerId, float x, float y, float z, int state)
{
    for (auto& player : skinned_object_list)
    {
        if (player->GetID() == playerId)
        {
            player->UpdatePosition(x, y, z, state);
            return;
        }
    }
}
